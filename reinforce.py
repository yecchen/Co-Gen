...
parser.add_argument('--sl_epoch', type=int, default=-1)
...
if args.sl_epoch != 0:
    rl_config["sl_epoch"] = args.sl_epoch
...
sl_config.saved_path = saved_path
...
if sl_config.use_gpu:
    model.cuda()
best_epoch = rl_config.sl_epoch
if args.sl_epoch != 0:
    best_epoch = args.sl_epoch
    rl_config.sl_epoch = best_epoch

model.load(pretrain_path, best_epoch)
model.print_summary()
logger.info("\n***** $$$ Load {}-model *****".format(best_epoch))
